...
import { X, BarChart2, Eye, Users, TrendingUp, ArrowUpRight, AlertCircle } from "lucide-react";
...
export default function EventStatsModal({ event, onClose }: EventStatsModalProps) {
      ...
      <div className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-2xl bg-[#f9fafb] dark:bg-zinc-950 border border-border shadow-2xl">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-gray-100 dark:border-zinc-800 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md px-6 py-4">
          <div>
             ...
             <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Detailní analytický pohled na výkon akce</p>
          </div>
          <button
            onClick={onClose}
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie 
                          ...
                          stroke="none"
                        >
                          {stats.genderDemographics.map((_: unknown, i: number) => (
